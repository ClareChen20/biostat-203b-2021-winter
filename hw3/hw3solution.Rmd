...
icustays_tble %>% 
  ...
  ungroup() %>%
  left_join(admissions_tble, by = c("subject_id", "hadm_id"), copy = TRUE) %>%
...
# icustays_tble %>%
#   # Find first ICU stay of each unique patient
#   group_by(subject_id) %>% 
#   filter(rank(intime) == 1) %>% 
#   ungroup() %>% 
#   # Left join admissions & patients tables
#   left_join(patients_tble, by = c("subject_id")) %>%
#   left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>%
#   # Select for the adults 
#   mutate(age_at_adm = year(admittime) - anchor_year + anchor_age) %>%
#   filter(age_at_adm >= 18) %>%
