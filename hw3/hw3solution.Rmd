...
```{r}
icustays_tble %>%
  # 1. Find first ICU stay of each unique patient
  group_by(subject_id) %>%
  filter(rank(intime) == 1) %>%
  ungroup() %>%
  # 2. Left join the patients & admissions
  left_join(patients_tble, by = c("subject_id")) %>%
  left_join(admissions_tble, by = c("subject_id", "hadm_id")) %>%
  # 3. Keep the adults
  mutate(age_at_adm = year(admittime) - anchor_year + anchor_age) %>%
  filter(age_at_adm >= 18) %>%
  # 4. Left join the labevents & chartevents
  left_join(labevents_tble, by = c("subject_id", "hadm_id")) %>%
  left_join(chartevents_tble, by = c("subject_id", "hadm_id", "stay_id")) %>%
  # 5. Display the SQL query
  show_query() %>%
  # 6. Collect the query
  collect() %>%
  # 7. Create an indicator for 30-day mortality
  mutate(deathin30 = ifelse(deathtime - admittime <= 43200, "Yes", "No")) %>%
    # We can choose to convert the missing values to "No",
    # But I think it makes more sense not to since we would need to predict the 30-days mortaily rate later
    # mutate_at(vars(deathin30), 
    #        function(x){ifelse(is.na(x) == TRUE, "No", x)}) %>%
  # 8. Save the file tibble
  saveRDS(str_c("./mimiciv_shiny/data/icu_cohort.rds"))
```
