...
``` {bash}
zcat /usr/203b-data/mimic-iv/core/admissions.csv.gz|awk -F ',' '{print $1}' |sort|uniq -c|wc -l 
```
...
Please kindly note that the vi command is extremely slow for me to execute on the teaching server, probably because of the VPN setting. I have tried the exact same code on my local terminal and it works just fine. So I did not execute theses two commands but only demonstrate them below.
